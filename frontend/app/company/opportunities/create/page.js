...
export default function CreateOpportunityPage() {
  ...
  const [form, setForm] = useState({ title: "", description: "", type: "field", category: "", location: "", requirements: "", positions: 1, deadline: "" });
  ...
  return (
    <div className="page-container py-8 md:py-10">
      ...
      <div className="mt-5 animate-fade-in">
        <span className="section-eyebrow">New Posting</span>
        <h1 className="text-3xl font-bold" style={{ color: "var(--text-primary)" }}>Post a New Opportunity</h1>
        <p className="mt-1 text-sm" style={{ color: "var(--text-secondary)" }}>Fill in the details below to attract the right students.</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="card mt-6 animate-fade-in rounded-2xl p-8">
              ...
              {types.map(({ value, icon, label }) => {
                ...
                return (
                  <button key={value} type="button" onClick={() => setForm((p) => ({ ...p, type: value }))}
                    ...
                    style={{ borderColor: active ? "#e0a800" : "var(--border)", background: active ? "#fffbeb" : "var(--surface)", boxShadow: active ? "0 0 0 3px rgba(40,167,69,0.1)" : "none" }}>
                    {active && (
                      <span className="absolute right-2 top-2 flex h-5 w-5 items-center justify-center rounded-full text-xs text-white"
                        style={{ background: "#e0a800" }}>✓</span>
                    )}
                    <span className="text-2xl">{icon}</span>
                    <span className="text-sm font-semibold" style={{ color: active ? "#b38600" : "var(--text-primary)" }}>{label}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input id="title" type="text" name="title" value={form.title} onChange={handleChange}
                  placeholder="e.g. Software Development Internship" required className="field-input" />
              </div>
              <div>
                <label className="field-label" htmlFor="description">Description</label>
                <textarea id="description" name="description" value={form.description} onChange={handleChange}
                  rows={7} placeholder="Describe what the student will be doing…" required className="field-input resize-y" />
              </div>
            </div>
          </Section>

          <Section title="Opportunity Details" desc="Specific information students need before applying.">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="field-label" htmlFor="category">Category</label>
                <input id="category" type="text" name="category" value={form.category} onChange={handleChange}
                  placeholder="e.g. IT, Accounting, Engineering" required className="field-input" />
              </div>
              <div>
                <label className="field-label" htmlFor="location">Location</label>
                <input id="location" type="text" name="location" value={form.location} onChange={handleChange}
                  placeholder="e.g. Dar es Salaam" required className="field-input" />
              </div>
              <div>
                <label className="field-label" htmlFor="positions">Number of Positions</label>
                <input id="positions" type="number" name="positions" min="1" value={form.positions} onChange={handleChange} required className="field-input" />
              </div>
              <div>
                <label className="field-label" htmlFor="deadline">Application Deadline</label>
                <input id="deadline" type="date" name="deadline" value={form.deadline} onChange={handleChange} required className="field-input" />
              </div>
            </div>
          </Section>

          <Section title="Requirements" desc="What skills or qualifications should applicants have?">
            <textarea name="requirements" value={form.requirements} onChange={handleChange} rows={7}
              placeholder={"- Studying Computer Science or related field\n- Basic programming knowledge\n- Good communication skills"}
          ...
          <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button type="submit" disabled={loading} className="btn btn-lg" style={{ background: "var(--brand-600)", color: "#fff" }}>
              {loading ? (
